...
![GTM definitions and relations](/blog/gtm-diagram.webp)

**GTM Functions** sit at the top. Each function has channels underneath it — the specific platforms or mediums where you execute.

**Actions** are the atomic unit. An action belongs to one channel. Channels belong to a GTM function. So actions have a many-to-one relationship with channels.

**Campaigns** are completely independent — a campaign doesn't belong to a channel. A campaign has a one-to-many relationship with actions. It can be a combination of actions across multiple channels, or just one channel. As simple as one post, or as complex as a coordinated push hitting five channels simultaneously.

**ICP** only comes in at the campaign level — when you're deciding who you're going after. GTM functions exist on their own, they don't require an ICP. An ICP has a one-to-many relationship with campaigns: you can have multiple campaigns going after the same persona, and you'll need separate campaigns for different ICPs.
